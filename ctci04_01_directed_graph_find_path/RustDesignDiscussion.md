...
- graph algorithms such as path finding, shortest path, spanning tree, etc.
- printing nodes and edges and potentially the entire graphs in JSON format into a string
- reading and persisting data from a simple file in JSON format

Full disclosure: There is of course a library already available that provides you with graph algorithms (TODO: link). We are not using this library because we want to practice our coding skills.
...
Based on the problem statement above, let's create some APIs. In C++ does not provide interfaces, but we can use pure virtual classes instead:
